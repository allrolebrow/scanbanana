...
# Paksa pakai opencv-python-headless, hindari import libGL
...
# Import cv2 dengan fallback
try:
    import cv2
except ImportError as e:
    st.error(f"OpenCV tidak dapat dimuat: {e}")
    st.stop()
...
.hero-title em { color: #C6D96A; font-style: italic; }
...
[data-testid="stTabs"] [role="tablist"] { gap: 0.5rem; margin-bottom: 1rem; }
...
hr { border: none; border-top: 1px solid #232623; margin: 1.5rem 0; }
</style>
...
def load_model():
    try:
        ...
        if not os.path.exists(model_path):
            st.error("File model tidak ditemukan di `model/best.pt`.")
            return None
...
def infer(model, img_array, conf, imgsz=640):
    t0 = time.time()
    # Kirim sebagai RGB array langsung ke YOLO (hindari cv2 BGR conversion jika bisa)
    res = model.predict(img_array, conf=conf, imgsz=imgsz, verbose=False)
    return res, time.time() - t0

def get_annotated(img_rgb, res):
    # plot() YOLO mengembalikan BGR numpy array
    bgr = res[0].plot(line_width=2, font_size=11, labels=True, conf=True)
...
def parse(res):
    ...
    for box in b:
        ...
        out.append({"label": norm(lbl), "raw": lbl, "conf": float(box.conf[0])})
...
with m1:
    st.markdown('<div class="metric-card"><div class="metric-value">3</div><div class="metric-label">Kelas</div></div>', unsafe_allow_html=True)
with m2:
    st.markdown('<div class="metric-card"><div class="metric-value">YOLOv8n</div><div class="metric-label">Model</div></div>', unsafe_allow_html=True)
with m3:
    st.markdown('<div class="metric-card"><div class="metric-value">Realtime</div><div class="metric-label">Inferensi</div></div>', unsafe_allow_html=True)
...
with tab1:
    ...
    with L:
        st.markdown('<div class="card"><div class="card-title">Input</div><div class="card-sub">Upload foto pisang</div>', unsafe_allow_html=True)
        uploaded = st.file_uploader("Upload gambar", type=["jpg","jpeg","png","webp","bmp"], label_visibility="collapsed")
        run = False
        if uploaded:
            ...
            st.markdown('<div class="empty-box"><h4>Belum ada gambar</h4><p>Format: JPG, PNG, WebP, BMP</p></div>', unsafe_allow_html=True)
        st.markdown("</div>", unsafe_allow_html=True)

    with R:
        st.markdown('<div class="card"><div class="card-title">Output</div><div class="card-sub">Hasil deteksi</div>', unsafe_allow_html=True)
        if uploaded and run:
            with st.spinner("Menganalisis gambar..."):
                arr = np.array(pil)
                res, t = infer(model, arr, conf_thresh, img_size)
                ann = get_annotated(arr, res)
                dets = parse(res)
            ...
            st.markdown('<div class="empty-box"><h4>Hasil akan muncul di sini</h4><p>Upload gambar lalu klik tombol analisis.</p></div>', unsafe_allow_html=True)
        st.markdown("</div>", unsafe_allow_html=True)
...
with tab2:
    ...
    with L2:
        st.markdown('<div class="card"><div class="card-title">Kamera</div><div class="card-sub">Ambil foto langsung</div>', unsafe_allow_html=True)
        ...
        if cam:
            ...
            cam_run = False
            st.markdown('<div class="empty-box"><p>Pastikan pisang terlihat jelas dan pencahayaan cukup.</p></div>', unsafe_allow_html=True)
        st.markdown("</div>", unsafe_allow_html=True)

    with R2:
        st.markdown('<div class="card"><div class="card-title">Output</div><div class="card-sub">Hasil deteksi kamera</div>', unsafe_allow_html=True)
        if cam and cam_run:
            with st.spinner("Menganalisis foto..."):
                pil2 = Image.open(cam).convert("RGB")
                arr2 = np.array(pil2)
                res2, t2 = infer(model, arr2, conf_thresh, img_size)
                ann2 = get_annotated(arr2, res2)
                dets2 = parse(res2)
            st.image(ann2, use_container_width=True)
            render_result(dets2, t2)
        else:
            st.markdown('<div class="empty-box"><h4>Belum ada hasil</h4><p>Ambil foto lalu klik tombol analisis.</p></div>', unsafe_allow_html=True)
        st.markdown("</div>", unsafe_allow_html=True)
